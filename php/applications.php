				...
				while($k = mysqli_fetch_array($aRes)) {
					...
					print('
                        <div class = "application-card-wrap">
                            <div class="application-card" id="application'.$k['ID'].'" style: "display: block;">
                                ...
                                <p><span>Телефон студента:</span> '.$k['stNumber'].'</p>
                                ...
                                <p><span>Номер родителя:</span> '.$k['prNumber'].'</p>
                    ...
                    if(empty($_GET['action']) || (!empty($_GET['action']) && $_GET['action'] != 'update_application_form')) {
                        print('
                            <div class = "admin-post-page-buttons">
                                <a href="index.php?page=applications&action=update_application_form&ID='.$k['ID'].'#application_form">
                                    Изменить
                                </a>
                            </div>');
                        }
					if($admin && !empty($_GET['action']) && ($_GET['action']=='update_application_form' && !empty($_GET['ID'])&&($_GET['ID']==$k['ID']))) {
				         print(' 
				         		<div class="update-application-form" id="application_form">
                                    <h3 class="text">Изменение заявки:</h3>
								    <form action="index.php?page=applications&action=update_application&ID='.$k['ID'].'#application'.$k['ID'].'" method="POST"> 
