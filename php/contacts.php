<?php
    ...
    ($_GET['action']=='update_message' && !empty($_GET['ID']))) {
        ...
        echo '
            <script>
                window.location.replace("index.php?page=contacts#message")
            </script>';
    }
    ...
    && ($_GET['action']=='delete_message') && !empty($_GET['ID'])) {
        ...
        echo '
            <script>
                window.location.replace("index.php?page=contacts#messages")
            </script>';
    }
    ...
    if(!empty($_GET['action'])&&($_GET['action']=='send_message')) {
        ...
        echo '
            <script>
                window.location.replace("index.php?page=contacts&showModal=true")
            </script>';
    }
?>
...
<?php
    ...
    $sql = "select * from Messages";
		...
		if($admin && $r = mysqli_query($db,$sql)) {
				print('
					<div class="applications-container container" id="messages">
				...
				while($k = mysqli_fetch_array($r)) {
					print('
                        <div class = "application-card-wrap">
                            <div class="application-card" style: "display: block;">
                                ...
                                <p><span>Телефон:</span> '.$k['number'].'</p>
                                </p>
                            ...
                            if(empty($_GET['action']) || (!empty($_GET['action']) && $_GET['action'] != 'update_message_form')) {
                                ...
                                <div class = "admin-post-page-buttons">
                                    <a href="index.php?page=contacts&action=update_message_form&ID='.$k['ID'].'#update_form">
                                        Изменить
                                    </a>
                                </div>');
                            }
					if($admin && !empty($_GET['action']) && ($_GET['action']=='update_message_form' && !empty($_GET['ID'])&&($_GET['ID']==$k['ID']))) {
				         print(' 
				         		<div class="update-application-form" id="update_form">
                                    <h3 class="text">Изменение заявки:</h3>
								    <form action="index.php?page=contacts&action=update_message&ID='.$k['ID'].'#message" method="POST"> 
